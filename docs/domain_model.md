...
Архитектура: микросервисы. Нас трое, поэтому пилим 2 сервиса: User & Catalog Service (юзеры и библиотека) и Tracking Service (сессии и очки).
Стек: бэкенд на C#, база PostgreSQL, так как сущности нормально ложатся в реляционку.
Взаимодействие: с клиентами по REST API, между сервисами по gRPC.
Надежность: для старта и стопа сессий нужен SLA 9.
Пограничные случаи: будем проверять, чтобы нельзя было стартовать вторую сессию при активной первой, ну и подобные вещи.
